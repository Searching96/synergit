...
import { OcticonFileAdded, OcticonFileChanged, OcticonFileRemoved } from "../../icons/Octicons";
...
function FileStatusIcon({ status }: { status: FileStatus }) {
  if (status === "added") return <OcticonFileAdded size={14} className="text-green-600 shrink-0" />;
  if (status === "removed") return <OcticonFileRemoved size={14} className="text-red-600 shrink-0" />;
  return <OcticonFileChanged size={14} className="text-[var(--text-secondary)] shrink-0" />;
}
