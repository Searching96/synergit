...
const OVERVIEW_ROWS = [
  ...
  },
  {
    ...
    statusColor: "text-[var(--accent-line)]",
  },
  {
  ...
  },
  {
    ...
    statusColor: "text-[var(--text-warning)]",
  },
  {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)] gap-4">
      <aside className="border border-[var(--border-muted)] rounded-md bg-[var(--surface-canvas)] py-3">
        <p className="px-4 text-[40px] leading-[1.2] font-semibold text-[var(--text-primary)] mb-3">Security and quality</p>
        <button
          type="button"
          className="w-full px-4 py-2 text-left text-sm font-semibold text-[var(--text-primary)] border-l-2 border-[var(--text-link)] bg-[var(--surface-subtle)] inline-flex items-center gap-2"
        >
          <Shield size={15} />
          Overview
        </button>
        ...
        {LEFT_SECTIONS.map((section) => (
          <div key={section.title} className="mt-3">
            <p className="px-4 text-xs font-semibold uppercase tracking-wide text-[var(--text-secondary)]">{section.title}</p>
            <div className="mt-1">
              {section.items.map((item) => (
                <button
                  key={item}
                  type="button"
                  className="w-full px-4 py-1.5 text-left text-sm text-[var(--text-secondary)] hover:bg-[var(--surface-subtle)]"
                >
                  {item}
                </button>
              ))}
      ...
      <section className="space-y-4 min-w-0">
        <h2 className="text-[44px] leading-[1.2] font-semibold text-[var(--text-primary)]">Overview</h2>
          ...
          {OVERVIEW_ROWS.map((row, index) => (
            ...
            >
              <div className="flex items-center gap-2 text-base font-semibold text-[var(--text-primary)]">
                {row.status === "Enabled" ? (
                  <ShieldCheck size={16} className="text-[var(--accent-line)]" />
                ) : row.status === "Needs setup" ? (
                  <AlertTriangle size={16} className="text-[var(--text-warning)]" />
                ) : (
