...
## Acceptance Criteria
- [ ] `/owner/repo/graphs/contributors?from={threeMonthsAgo}` is the default Contributors tab URL.
- [ ] `/owner/repo/graphs/contributors?all=1` renders all default-branch non-merge commits.
- [ ] `/owner/repo/graphs/contributors?from=...` renders Last month or Last 3 months based on the matching date window.
- [ ] Period dropdown updates URL to GitHub-style query strings.
- [ ] Contributions dropdown displays `Commits` and does not imply unsupported metrics.
- [ ] Charts and contributor cards closely match the supplied GitHub screenshot.
...
- Implemented default-branch, non-merge commit aggregation with period filtering and contributor sorting.
- Added frontend routing for `/:owner/:repo/graphs/contributors` and preserved GitHub-style query strings.
- Added Contributors view under Insights with Period and Contributions dropdowns, overview chart, and contributor cards.
