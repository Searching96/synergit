...
## Acceptance Criteria
- [ ] Backend: Update `port.GitManager` and `LocalGitAdapter` to support `limit` and `offset` for commit log traversal.
- [ ] Backend: Update the `GetCommits` HTTP handler to parse pagination query parameters and return a paginated response (including `total_commits`).
- [ ] Frontend: Update `reposApi.getCommits` to accept pagination parameters.
- [ ] Frontend: Update `CommitHistory.tsx` to display an "Older" button to fetch and append the next page of commits.
